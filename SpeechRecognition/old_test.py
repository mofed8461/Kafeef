...
fs, sub = wav.read('/home/pi/Desktop/Code/SpeechRecognition/test.wav')

for s in sub:
    audio.append(np.short((int(s))))
...
inputs = mfcc(audio, samplerate=fs, nfft=2048)
...
with tf.Session(graph=graph) as session:
    ...
    saver.restore(session, "/home/pi/Desktop/Code/SpeechRecognition/data/ynxc.ckpt")
    ...
    if ('e' in str_decoded):
        exit(0)
    elif ('y' in str_decoded):
        exit(1)
    elif ('n' in str_decoded):
        exit(2)
    elif ('x' in str_decoded):
        exit(3)
    elif ('c' in str_decoded):
        exit(4)

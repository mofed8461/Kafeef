...
def wave_batch_generator(batch_size=10,source=Source.DIGIT_WAVES,target=Target.digits): #speaker
    ...
    for i in range(0, len(wordFiles)):
        ...
        words.append(str(fileNames[i].replace('empty', 'e').replace('read', 'x').replace('record', 'c').replace('.wav', '').replace('1', '').replace('2', '').replace('3', '').replace('4', '').replace('5', '').replace('6', '').replace('7', '').replace('8', '').replace('9', '').replace('0', '')))

    """
    #ccchash = ['e':empty, 'x':read, 'c':record, 'y':yes, 'n':no, 'h':happy, 's':sad, 'f':search face]

    ccchash = ['e', 'x', 'c', 'y', 'n', 'h', 's', 'f']
    ...
    while True:
        ...
        for wav in files:
            ...
            xid = ccchash.index(str(wav.replace('empty', 'e').replace('read', 'x').replace('record', 'c').replace('.wav','').replace('1','').replace('2', '').replace('3', '').replace('4', '').replace('5', '').replace('6', '').replace('7', '').replace('8', '').replace('9', '').replace('0', '')))

            labels.append(dense_to_one_hot(int(xid)))

            chunk = load_wav_file(folder+wav)
